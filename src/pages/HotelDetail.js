...
const HotelDetail = () => {
    ...
    const fetchHotelReviews = async () => {
        try {
            const response = await get(`/api/accommodation/feedback-review/accommodation/${id}/`);
            if (response.code === 200 && response.data) {
    ...
    const fetchGuestServices = async () => {
        try {
            const response = await get(`/api/accommodation/guest-service/guestService/${id}/`);
            if (response.code === 200 && response.data) {
                setGuestServices(response.data);
            } else {
                throw new Error(response.msg || "获取客房服务失败");
            }
        } catch (err) {
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="lg:w-2/3">
                    <div className="bg-white shadow-lg rounded-lg overflow-hidden">
                        ...
                        <div className="p-6">
                            ...
                            <p className="text-gray-700 mb-6 text-lg leading-relaxed">
                                {hotel.description || t('defaultHotelDescription')}
                            </p>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                ...
                                <div className="flex items-center bg-gray-100 p-4 rounded-lg">
                                    ...
                                    <div>
                                        ...
                                        <p>{hotel.types.map(typeId => roomTypes[typeId]?.room_type).join(', ')}</p>
                                    </div>
                                </div>
                                <div className="flex items-center bg-gray-100 p-4 rounded-lg">
                                    ...
                                    <div>
                                        ...
                                        <p>{t('checkIn')}: {hotel.check_in_time}</p>
                                        <p>{t('checkOut')}: {hotel.check_out_time}</p>
                                    </div>
                                </div>
                                <div className="flex items-center bg-gray-100 p-4 rounded-lg">
                            ...
                            <div>
                                <h3 className="text-xl font-semibold mb-4">{t('facilitiesAndServices')}</h3>
                                <div className="bg-gray-100 p-4 rounded-lg">
                                    <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                                        {hotel.amenities.split(',').map((amenity, index) => (
                                            <li key={index} className="flex items-center">
                                                <span className="w-2 h-2 bg-blue-500 rounded-full mr-2 flex-shrink-0"></span>
                                                <span className="text-gray-700 text-sm">{amenity.trim()}</span>
                                            </li>
                                        ))}
                                    </ul>
                                </div>
                    ...
                    <div className="mt-8 bg-white shadow-lg rounded-lg overflow-hidden">
                        <div className="p-6">
                            <h3 className="text-2xl font-semibold mb-4">{t('roomService')}</h3>
                            {guestServices.length > 0 ? (
                                <ul className="space-y-4">
                                    {guestServices.map((service) => (
                                        <li key={service.id} className="flex items-center justify-between border-b pb-4">
                                            <div>
                                                <h4 className="text-lg font-medium">{service.service_name}</h4>
                                                <p className="text-sm text-gray-600">{service.availability_hours}</p>
                                            </div>
                                ...
                                <p className="text-gray-600">{t('noRoomServiceInfo')}</p>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:w-1/3">
                    <BookingModule hotelId={id} roomTypes={roomTypes} isLoggedIn={!!user} />
                    <div className="bg-white shadow-lg rounded-lg overflow-hidden">
                        <div className="p-6">
                            ...
                            {user && (
                                <form onSubmit={handleSubmitReview} className="mb-8">
                                    <h3 className="text-xl font-semibold mb-4">{t('addReview')}</h3>
                                    <div className="mb-4">
                                        <label className="block text-gray-700 text-sm font-bold mb-2">{t('rating')}</label>
                                        <div className="flex">
                                            {[1, 2, 3, 4, 5].map((star) => (
                                                <StarIcon
                                                    key={star}
                                    ...
                                    <div className="mb-4">
                                        <label htmlFor="review" className="block text-gray-700 text-sm font-bold mb-2">{t('review')}</label>
                                        <textarea
                                            id="review"
                                            rows="4"
                            ...
                            {/* 评论列表 */}
                            <h3 className="text-xl font-semibold mb-4">{t('userReviews')}</h3>
                            {reviews.length > 0 ? (
                                reviews.map((review) => (
                                    <div key={review.id} className="mb-4 pb-4 border-b border-gray-200 last:border-b-0">
                                        <div className="flex items-center justify-between mb-2">
                                            <div className="flex items-center">
                                                ...
                                                <span className="ml-2 text-gray-600">({review.rating} {t('stars')})</span>
                                            </div>
                                            {userId && userId === review.user.toString() && (
                                                <button
                                        ...
                                        <p className="text-sm text-gray-500">
                                            ...
                                            {userId && userId === review.user.toString() ? (
                                                <span className="ml-2 text-blue-500">({t('self')})</span>
                                            ) : (
                                                <span className="ml-2 text-gray-500">({t('anonymous')})</span>
                                            )}
                                        </p>
                                    </div>
                                ))
